...
~~因为我不会把 app 包名转换为 app 名称，所以借小米应用商店用了一下，名称解析的结果都会保存到 cache.txt，因为小米应用商店的限制，部分应用还是获取不到名称，~~
现在使用aapt获取应用名称，如果你不是arm64架构请 *~~自行寻找~~* [下载](https://github.com/wahyu6070/aapt)对应的可执行文件放入模块根目录替换  
可以在 cache.txt 里面手动指定,格式: `包名=名称`
