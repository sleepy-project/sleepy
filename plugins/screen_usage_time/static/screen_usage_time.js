...
class ScreenUsageTime {
    ...
    render() {
        ...
        const { devices, last_updated } = this.data;
    ...
    renderDevices(devices) {
        if (!devices || Object.keys(devices).length === 0) {
            return '<div class="device-card">暂无设备数据</div>';
        }
    ...
    renderUsageItems(items, device_id) {
        ...
        return sortedItems.map(([id, data]) => {
            const { name, icon, formatted_time, progress } = data;
            const displayName = name || id;
            let iconHtml = '';
            
            if (icon) {
                iconHtml = `
                    <img src="/plugin/screen_usage_time/icons/${device_id}/${icon}" alt="${displayName}" onerror="this.style.display='none';this.nextElementSibling.style.display='block';">
